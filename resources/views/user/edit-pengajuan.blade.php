...
<body class="m-0 font-sans bg-[#F0F4F8] text-gray-800 min-h-screen">
    ...
    <main class="max-w-3xl mx-auto px-6 lg:px-8 py-8">
        ...
        <div class="bg-[#006A97] rounded-t-2xl px-6 py-5">
            <h1 class="text-white text-xl font-bold">Revisi Pengajuan Konten</h1>
        ...
        <div class="bg-white rounded-b-2xl shadow-sm border border-gray-100 border-t-0 px-6 py-6">
            ...
            @method('PUT')
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $publication->judul) }}"
                ...
                <div class="grid grid-cols-2 gap-4 mb-5">
                    <div>
                        ...
                        <select name="jenis_konten"
                            ...
                            <option value="" disabled {{ old('jenis_konten', $publication->jenis_konten) ? '' : 'selected' }}>Pilih kategori</option>
                            @foreach(['Prestasi Mahasiswa', 'Kegiatan Organisasi', 'Berita Akademik', 'Lainnya'] as $k)
                            <option value="{{ $k }}" {{ old('jenis_konten', $publication->jenis_konten) === $k ? 'selected' : '' }}>{{ $k }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="date" name="tanggal_kegiatan" value="{{ old('tanggal_kegiatan', optional($publication->tanggal_kegiatan)->format('Y-m-d')) }}"
                            class="w-full h-12 rounded-xl border border-gray-300 bg-white px-4 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#006A97]/30 focus:border-[#006A97] transition
                ...
                <div class="mb-5">
                    ...
                    <textarea name="deskripsi" rows="4"
                        ...
                        {{ $errors->has('deskripsi') ? 'border-red-400 bg-red-50' : '' }}"> {{ old('deskripsi', $publication->deskripsi) }}</textarea>
                </div>
